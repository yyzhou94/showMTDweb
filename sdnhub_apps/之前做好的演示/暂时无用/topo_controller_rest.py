...
simple_switch_instance_name = 'topo_controller'
url = '/simpleswitch/topo'

class TopoDiscoverRest(app_manager.RyuApp):
    ...
    _CONTEXTS = {
        ...
        # 'dpset': dpset.DPSet,
        'topo_controller':topo_controller.TopoDiscover
    }

    def __init__(self, *args, **kwargs):
        super(TopoDiscoverRest, self).__init__(*args, **kwargs)
        self.switches = {}
        wsgi = kwargs['wsgi']
        # dpset = kwargs['dpset']
        topo_controller = kwargs['topo_controller']
        self.waiters = {}
        self.data = {}
        # self.data['dpset'] = dpset
        self.data['waiters'] = self.waiters
        self.data['topo_controller'] = topo_controller



        wsgi.register(TopoDiscoverController, {simple_switch_instance_name : self})

        mapper = wsgi.mapper
        mapper.connect('topo', '/v1.0/disco/topo',
                       controller=TopoDiscoverController, action='enable_topo',
                       conditions=dict(method=['POST']))

        # mapper.connect('topo_controller', '/simpleswitch/version',
...
class TopoDiscoverController(ControllerBase):

    def __init__(self, req, link, data, **config):
        super(TopoDiscoverController, self).__init__(req, link, data, **config)
        self.topo_controller = data[simple_switch_instance_name]
    ...
    def enable_topo(self,req, **kwargs):
        for i in range(1,9):
        ...
        if topo_controller.topo_disco_enable == True:
            topo_controller.topo_disco_enable = False
        else:
            topo_controller.topo_disco_enable = True
        print "topo_controller.topo_disco_enable = %s "  %topo_controller.topo_disco_enable
        return Response(status=200,content_type='application/json',
                    body=json.dumps({'status':'success'}))