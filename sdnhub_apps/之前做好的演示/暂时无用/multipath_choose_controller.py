...
class MultipathChoose(app_manager.RyuApp):
    ...
    def create_interior_links(self, link_list):
        ...
        if (switch_port_1[0] in self.switches and switch_port_2[0] in self.switches):
            # print '*************deal with the other switch*****************'
    ...
    def shortest_forwarding(self, msg, header_list):
        ...
        if src_dst_sw:
            ...
            if dst_sw:
                ...
                # self.install_flow(*args)
                choose_paths = self.get_appropriate_paths_by_bw(self.free_bandwidth, paths)
                if len(choose_paths) > 1:
                    self.multipath_table.setdefault((ip_src, ip_dst), [0,[]] )
                    self.multipath_table[(ip_src, ip_dst)][1] = choose_paths
                self.install_flow(self.datapaths,
                                  self.link_to_port,
                                  self.access_table, choose_paths[0],
    ...
    def install_flow(self, datapaths, link_to_port, access_table, path,
        ...
        # in_port 代表调用该函数的是packe_in还是其他
        if in_port != 0:
            # print '*********in install fuction, in_port is not zero**********'
